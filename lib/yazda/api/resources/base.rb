...
require 'multi_json'
require 'yazda/api/errors/errors'

module Yazda::Api
  module Common
    # Make a call directly to the Yazda API.
    # (See any of the other methods for example invocations.)
    #
    # @param path the Yazda API path to query (no leading / needed)
    ...
    #
    # @raise [Yazda::Api::APIError] if Yazda returns an error
    #
    # @return the result from Yazda
    ...
    # Makes a request to the appropriate Yazda API.
    ...
    #
    # @raise [Yazda::Api::ServerError] if Yazda returns an error (response status >= 500)
    #
    # @return the body of the response from Yazda
    def api(path, args = {}, verb = "get", options = {}, &error_checking_block)
      ...
      args["access_token"] ||= self.access_token ||= Yazda::Api::Configuration::DEFAULT_ACCESS_TOKEN
      endpoint ||= self.endpoint ||= Yazda::Api::Configuration::DEFAULT_ENDPOINT
      ...
      result = HTTMultiParty.public_send(verb, "#{endpoint}#{path}", query: args)

      if result.code.to_i >= 500
        raise Yazda::Api::ServerError.new(result.code.to_i, result.body)
      end
    ...
    # Sanitizes Ruby objects into Yazda-compatible string values.
    #
